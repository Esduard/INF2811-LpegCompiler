...
lua5.1 main.lua < "$directory/$filename.txt" > main.ll 2> "$directory/$filename.err"

# Check if the error file has content
if [ -s "$directory/$filename.err" ]; then
...
if [ -f "main.ll" ]; then
    ...
    if [[ "$first_line" == "syntax error near"* ]]; then
        ...
        cat "main.ll"
        rm -f "$directory/$filename.err" "$directory/$filename.res"
        exit 1
    else
